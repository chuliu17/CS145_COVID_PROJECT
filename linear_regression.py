...
def getDataframe(filePath):
    ...
    y = dataframe['y']
    x = dataframe.drop('y', axis=1) #drop column y
    return x, y

# Reads the data from CSV files, converts it into Dataframe and returns x and y dataframes
def getTrainDataframe(filePath, state, target_y ):
    df = pd.read_csv(filePath)
    basedate = pd.Timestamp('2020-01-01')
    df['Date'] = (pd.to_datetime(df['Date']) - basedate).dt.days
    #df = df.drop(['ID','Recovered',], axis=1) #drop column
    #df.drop('Recovered', axis=1, inplace=True)
    #df = df_.drop("Recovered", axis=1)
    state_df = df[df['Province_State']==state]
    y = state_df[target_y]
    #print(y)
    x = state_df[['Date']]
    #drop([target_y, 'ID','Province_State','Recovered' ], axis=1) #drop column
...
def getBeta(train_x, train_y):
    ...
    #========================#
    x_transpose = train_x.transpose()
    product = np.matmul(x_transpose, train_x)
    product_inverse = np.linalg.inv(product)

    right_side = np.matmul(x_transpose, train_y)
    beta = np.matmul(product_inverse,right_side)
...
def getBetaBatchGradient(train_x, train_y, lr, num_iter):
    ...
    for iter in range(0, num_iter):
       deriv = np.zeros(p)
       for i in range(n): # suumation of xi(xi^t * beta - yi)
           #========================#
           # STRART YOUR CODE HERE  #
           #========================#
           #print(beta.shape)
           x = train_x[i]
           x_beta = x.dot(beta) # 1 by 1
           #print(x_beta.shape)
           #print(x_beta.shape)
           xb_y = x_beta - train_y[i] #scalar

           #update deriv for xi
           deriv = np.add(deriv, xb_y*x)

           '''
           x = train_x[i].reshape(p,1) #need to change it to 2d so you can do matrix mult.
           beta = beta.reshape(p,1)
           #print(beta.shape)

           x_transpose = x.transpose()
           #print(x_transpose.shape)
           x_beta = np.matmul(x_transpose, beta) # 1 by 1
           #print(x_beta.shape)

           x_beta_scalar = np.asscalar(x_beta) #scalar

           xb_y = x_beta_scalar - train_y[i] #scalar

           #reshape x and beta backt to normal again
           x = x.reshape(p)
           beta = beta.reshape(p)
           ...
           #   END YOUR CODE HERE   #
           #========================#
       deriv = deriv / n #calculate gradient, dimension 1d: 101
       beta = beta - deriv.dot(lr) #lr is a scalar, same as lr*deriv
    return beta
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
           ...
           # STRART YOUR CODE HERE  #
           #========================#
            beta = beta + lr*(train_y[idx] - train_x[idx]*beta)*train_x[idx]
...
class LinearRegression(object):
    ...
    def load_project_data(self, train_file, state, target_y):
        #print(y)
        self.train_x, self.train_y = getTrainDataframe(train_file, state, target_y)
